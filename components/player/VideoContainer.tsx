...
export function VideoContainer() {
    ...
    const currentLesson = allLessons.find((l: any) => Number(l.id) === currentLessonId);
    const currentIndex = allLessons.findIndex((l: any) => Number(l.id) === currentLessonId);
    ...
    const handleNext = () => {
        if (currentIndex < allLessons.length - 1) {
            setCurrentLessonId(Number(allLessons[currentIndex + 1].id));
        }
    };
    ...
    const resumeFrom = mediaData?.resume_from_seconds ?? serverResume ?? undefined;
    ...
    const tracker = useVideoEngagementTracker({
        ...
        initialResumeSeconds: typeof resumeFrom === "number" ? resumeFrom : undefined,
        onShouldSync: ({ watchedSeconds, totalSeconds, lastPositionSeconds }) => {
            ...
            track.mutate(
                ...
                },
                {
                    onSuccess: (res) => {
                        if (res?.success) {
                            setServerResume(res.data.resume_from_seconds ?? null);
                        }
                    },
                }
            );
                ...
                {isLoadingMedia ? (
                    ...
                    <div className="absolute inset-0 flex items-center justify-center bg-black/90">
                        <div className="text-center">
                            ...
                            />
                            <p className="relative z-10 text-white/60 font-mono text-sm">
                ...
                <div className="flex flex-col gap-1">
                    ...
                    {!isCompleted && (
                        <div className="text-xs text-text/60">
                            شاهد 80% على الأقل لإكمال الدرس. السحب للأمام لا يُحسب ضمن المشاهدة.
                        </div>
                    )}
                </div>
