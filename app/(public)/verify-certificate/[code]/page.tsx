...
import { Award, CheckCircle, XCircle, Loader2, Copy } from "lucide-react";
...
export default function VerifyCertificatePage() {
    ...
    const verified = result?.verified && result?.data;
                        ...
                        {verified && result.data && (
                            <div className="w-full text-right space-y-3 pt-4 border-t border-border/50">
                                ...
                                <div className="flex justify-between gap-4">
                                    ...
                                    <span className="font-medium">{result.data.instructor_name}</span>
                                </div>
                                <div className="flex justify-between gap-4">
                                    <span className="text-text/60 text-sm">تاريخ الإكمال</span>
                                    <span className="font-medium">
                                        {new Date(result.data.completion_date).toLocaleDateString("ar-SA")}
                                    </span>
                                </div>
                                <div className="flex justify-between gap-4">
                                    <span className="text-text/60 text-sm">رقم الشهادة</span>
                                    <span className="font-mono text-xs">{result.data.certificate_code}</span>
                                </div>
                            </div>
                        )}
